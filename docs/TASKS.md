...
- `navigator.clipboard`はセキュアコンテキスト(HTTPSまたはlocalhost)でしか
  ...
  と案内する(実機のPlaywright検証で、この画面がまさに非セキュアコンテキスト
  になり得ることを実際に確認した)。
- 発行直後のキー表示バナーは、そのキーを一覧から失効させた後も(別のキーを
  新規発行するかページを再読み込みするまで)そのまま表示され続ける
  (直前に何を発行したかの記録として残すため、意図的な挙動)。
- 実機でPlaywright検証済み(発行→一覧反映→コピー失敗時の案内表示→失効)。
