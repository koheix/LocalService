...
バックエンド(`POST/GET/DELETE /api/auth/api-keys`、T-05で実装済み、無変更)は
そのままに、`/api-keys`のプレースホルダーを実装に置き換える。admin限定では
なく、ユーザーが自分自身のAPIキーを管理する画面(ホーム画面のAppGridから
既にリンクされている)。

- [x] 自分のAPIキー一覧(名前・prefix・作成日・最終利用日)を表示する
- [x] 名前を入力して新規発行できる
- [x] 発行直後だけ平文キーを表示する(以降は取得不可のため、コピー導線と
      「二度と表示されません」という警告を出す)
- [x] 失効(削除)できる

**完了条件**
ユーザーがAPIキー画面を開くと既存キー一覧が表示され、名前を入力して
発行すると平文キーが一度だけ表示される。失効もできる。

**実装メモ（完了）**
- `navigator.clipboard`はセキュアコンテキスト(HTTPSまたはlocalhost)でしか
  使えない。この画面は社内LAN上でHTTPのまま(IPアドレス/ホスト名で)
  アクセスされる可能性があるため、コピーボタンが使えない場合は
  「自動コピーに失敗しました。上のキーをクリックして全選択し、手動で
  コピーしてください」と案内する(実機のPlaywright検証で、この画面が
  まさに非セキュアコンテキストになり得ることを実際に確認した。D-019参照)。
- 発行直後のキー表示バナーには閉じるボタンを付けた(レビューで指摘: 当初
  閉じる手段が無く、キーを控え終えたユーザーが自分で消せなかった)。
  閉じる・別のキーを新規発行する・ページを再読み込みする、のいずれかまで
  表示され続ける。そのキーを一覧から失効させただけではバナーは消えない
  (直前に何を発行したかの記録として残すため)。
- 一覧の再取得(発行・失効の直後に行う)が失敗しても、発行・失効自体は
  成功しているため、それらとは別の「一覧の再取得に失敗しました」という
  メッセージにしている(レビューで指摘: 混同すると誤って重複発行され得る)。
- 実機でPlaywright検証済み(発行→一覧反映→コピー失敗時の案内表示→
  バナーを閉じる→失効)。

---

### T-31 プレイグラウンド・RAGに思考中(reasoning)のライブ表示を追加する

`chat-standard`(qwen3:4b)はOllama側のテンプレート設定で応答生成のたびに
強制的に長い思考(`<think>`)を行う(T-26で発覚、対応方針は保留中)。この
思考自体は止められないため、代わりに思考内容をリアルタイムでユーザーに
見せることで、待ち時間を「固まっている」ように見せないようにする
(ユーザー確認済み)。

OpenAI互換のstreamingレスポンスは`choices[].delta.reasoning`として
思考内容をトークン単位で配信する(`content`とは別フィールド)ことを実機で
確認済み。gatewayの`/api/v1`・`/api/conversations`のSSE中継は生のJSONを
素通しするため、この`reasoning`フィールドは**バックエンド無改修で**
フロントに届く。RAG(`/api/rag/query`)は独自のSSE契約のため、こちらは
gateway側にも対応が必要。

- [x] プレイグラウンド: ストリーミング中、`content`が空の間は思考内容を
      ライブ表示する。`content`が出始めたら思考部分を折りたたむ
- [x] RAG: `gateway/app/routers/rag.py`が`reasoning`もSSEイベントとして
      中継するようにする。フロント側もプレイグラウンドと同様に表示する
- [x] 折りたたみ後も「思考過程を見る」で再度展開できる

**完了条件**
プレイグラウンド・RAGのいずれでメッセージ/質問を送っても、回答本文が
出るまでの間、モデルの思考内容がリアルタイムで表示される。本文が出始めると
思考部分は折りたたまれ、回答が通常通り表示される。折りたたんだ思考は
再度展開して読める。

**実装メモ（完了）**
- 共有コンポーネント`console/src/components/ThinkingBox.tsx`を新設し、
  プレイグラウンド(`MessageList.tsx`)・RAG(`QuestionPanel.tsx`)の両方から
  使う形にした。
- プレイグラウンド・`/api/conversations`側は無改修(想定どおり、生SSEを
  素通ししているため`reasoning`フィールドも自動的に届く)。
- RAGは`gateway/app/routers/rag.py`の`_extract_stream_events`
  (旧`_extract_delta_event`から改名、1行から複数イベントを返せるように
  変更)で`reasoning`を`{"type":"reasoning","content":...}`イベントとして
  中継するようにした。テストも追加。
- 会話履歴の再取得ではreasoningは保存されない(意図的。ライブ中のみの
  一時的な表示)。
- `ThinkingBox`をanswerの有無で分岐する三項演算子の別々の枝に置くと、
  Reactが別インスタンスとして再マウントし、「done化した瞬間だけ自動で
  畳む」ロジックが働かなくなる不具合をレビューで発見・修正した
  (常に同じ位置・同じコンポーネント型で描画し、`done`propだけを
  変化させる形にした)。あわせて`ThinkingBox`自体も、万一done=trueの
  状態でいきなりマウントされた場合は直ちに畳むよう防御的にした。
- `reasoning`はOllama固有のフィールド名(vLLM等は別名を使うのが一般的)で、
  RAG側のルーター層(`rag.py`)がこの名前を直接読んでいる。Phase 4で
  `backends/vllm.py`に切り替える際は対応が必要(`docs/DECISIONS.md`の
  D-020参照)。
- 実機でPlaywright検証済み(プレイグラウンド・RAG両方で、思考内容が
  リアルタイムに伸びていくこと、本文が出始めると折りたたまれて
  「思考過程を見る」になること、引用表示までの一連の流れを確認)。
